...
const keyFor = (r: Row) => `${r.card_name}|${r.set_code ?? ''}|${r.collector_number ?? ''}`
...
export default function DeckViewPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const pairs = await Promise.all(keys.map(async k => {
        ...
        const info = await getCardInfoFor(name, set || undefined, num || undefined)
  ...
  return (
    <div className="container">
          ...
          return (
            <div key={group} className="card">
              ...
              {view === 'list' ? (
                <ul>
                  {rowsInGroup.map(r => (
                    <li key={keyFor(r)} style={{display:'flex', alignItems:'center', gap:8}}>
                  ...
                  {rowsInGroup.map(r => {
                    ...
                    return (
                      <div key={keyFor(r)} className="thumb-wrap">
                        ...
                        <button
                          type="button"
                          className="thumb as-button"
                          aria-label={`View ${r.card_name}`}
                          onClick={() => setOpenKey(keyFor(r))}
                        >
                          {info ? (
      ...
      {openKey && infos[openKey]?.normal && (
        <Lightbox
          src={infos[openKey]!.normal}
          alt={openKey.split('|')[0]}
